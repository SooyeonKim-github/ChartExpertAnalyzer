...
class TickerUniverse:
    """Use the same repository stock list as the other analyzers, but independent signals."""
    ...
    def load(self) -> pd.DataFrame:
        if not self.info_excel.exists():
            raise FileNotFoundError(
                f"종목 Universe Excel을 찾을 수 없습니다: {self.info_excel}. "
                "기본값은 SwingChartProbabilityAnalyzer/KOSPI_Info.xlsx 입니다."
            )
        ...
        etf_mask = market.str.contains("ETF|ETN|ETP", regex=True, na=False) | name.str.contains(
            "ETF|ETN|ETP", regex=True, na=False
    ...
    def get(self, top_n: int = 0, sort_by: str = "market_cap") -> list[TickerInfo]:
        df = self.load()
        if top_n > 0:
            sort_col = SORT_ALIASES.get(sort_by)
            if sort_col is None or sort_col not in df.columns:
                raise ValueError(
                    f"정렬 기준 컬럼 없음: {sort_by}; "
                    f"지원={list(SORT_ALIASES)} / Excel columns={list(df.columns)}"
                )
            df["_sort"] = _numeric(df[sort_col])
            df = df.sort_values("_sort", ascending=False).head(top_n)

        return [
            TickerInfo(str(r["Ticker"]).zfill(6), str(r["Name"]), str(r["Market"]))
