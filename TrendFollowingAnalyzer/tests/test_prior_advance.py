import numpy as np
import pandas as pd
from trend_following_analyzer.structure import INSUFFICIENT,RECENT_WINDOW_PROXY,compute_prior_advance_snapshot

def _frame(values):
    return pd.DataFrame({"close":values,"ma150":np.linspace(90,110,len(values))},index=pd.bdate_range("2025-01-01",periods=len(values)))
...
def test_recent_proxy_window_is_excluded():
    values=np.r_[np.linspace(100,70,40),np.linspace(70,120,80),np.linspace(120,300,20)]; s=compute_prior_advance_snapshot(_frame(values),lookback_sessions=100,recent_window_sessions=20); assert s.prior_peak<200
