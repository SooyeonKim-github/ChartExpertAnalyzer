...
def test_intraday_proxy_rules_are_experimental_and_backtest_only():
    keys={"market.intraday_daily_ohlc_proxy","market.intraday_clv_threshold","market.intraday_rolling_ratio","market.intraday_composite_score"}; rules=[r for r in RULE_CATALOG if r.key in keys]; assert {r.key for r in rules}==keys and all(r.origin==EXPERIMENTAL and r.mode==BACKTEST_ONLY for r in rules)
...
def test_relative_strength_numeric_rules_are_experimental():
    keys={"stock.relative_strength_price_ratio","stock.relative_strength_windows","stock.relative_strength_percentile","stock.relative_strength_percentile_threshold"}; rules=[r for r in RULE_CATALOG if r.key in keys]; assert {r.key for r in rules}==keys and all(r.origin==EXPERIMENTAL and r.mode==BACKTEST_ONLY for r in rules)
