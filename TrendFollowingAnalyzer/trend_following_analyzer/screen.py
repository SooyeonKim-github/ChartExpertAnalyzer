...
def _build_market_context(
    ...
    for market in ("KOSPI", "KOSDAQ"):
        ...
        try:
            ...
            intraday_history = compute_intraday_strength_history(
                index_df,
            ...
            intraday_snapshots[market] = latest_intraday_snapshot(
                intraday_history,
                market=market,
                short_window=int(icfg.get("short_window", 5)),
                long_window=int(icfg.get("long_window", 20)),
...
def _build_rs_context(
    ...
    for ticker, info in universe.iterrows():
        ...
        try:
            ...
            stock_df = provider.get_daily(code, resolved)
            benchmark_df = provider.get_market_index(market, resolved)
    ...
    if rows:
        ...
        for _, row in ranked.iterrows():
            ...
            percentile_map[str(row["ticker"]).zfill(6)] = {
...
def screen_date(
    ...
    *,
    ...
    market_snapshots, breadth_snapshots, intraday_snapshots = _build_market_context(
        provider, cfg, resolved
    )
    rs_snapshots, rs_percentiles = _build_rs_context(provider, cfg, resolved, universe)
    ...
    for ticker, info in universe.iterrows():
        try:
            ...
            rs_snapshot = rs_snapshots.get(code)
            rs_rank = rs_percentiles.get(code, {})

            if (
                market_snapshot is None
                or breadth_snapshot is None
                or intraday_snapshot is None
                or rs_snapshot is None
            ):
                continue
            ...
            results.append(
                StageScreenResult(
                    ...
                    market_intraday_status=intraday_snapshot.status,
                    market_intraday_measurement=intraday_snapshot.measurement,
                    market_gap_return_pct=intraday_snapshot.gap_return_pct,
                    market_open_close_return_pct=intraday_snapshot.open_close_return_pct,
                    market_close_return_pct=intraday_snapshot.close_return_pct,
                    market_close_location_value=intraday_snapshot.close_location_value,
                    market_recovery_strength_pct=intraday_snapshot.recovery_strength_pct,
                    market_fade_strength_pct=intraday_snapshot.fade_strength_pct,
                    market_intraday_label=intraday_snapshot.intraday_label,
                    market_experimental_intraday_strength_score=intraday_snapshot.experimental_intraday_strength_score,
                    market_weak_open_strong_close_5d_ratio=intraday_snapshot.weak_open_strong_close_5d_ratio,
                    market_weak_open_strong_close_20d_ratio=intraday_snapshot.weak_open_strong_close_20d_ratio,
                    market_strong_open_weak_close_5d_ratio=intraday_snapshot.strong_open_weak_close_5d_ratio,
                    market_strong_open_weak_close_20d_ratio=intraday_snapshot.strong_open_weak_close_20d_ratio,
                    market_strong_close_5d_ratio=intraday_snapshot.strong_close_5d_ratio,
                    market_strong_close_20d_ratio=intraday_snapshot.strong_close_20d_ratio,
                    market_weak_close_5d_ratio=intraday_snapshot.weak_close_5d_ratio,
                    market_weak_close_20d_ratio=intraday_snapshot.weak_close_20d_ratio,
                    market_intraday_direction=intraday_snapshot.intraday_direction,
                    market_intraday_filter_applied=False,
                    rs_status=rs_snapshot.status,
                    rs_measurement=rs_snapshot.measurement,
                    rs_percentile_scope=rs_snapshot.percentile_scope,
                    stock_return_20d_pct=rs_snapshot.stock_return_20d_pct,
                    benchmark_return_20d_pct=rs_snapshot.benchmark_return_20d_pct,
                    rs_20d_pct=rs_snapshot.rs_20d_pct,
                    stock_return_60d_pct=rs_snapshot.stock_return_60d_pct,
                    benchmark_return_60d_pct=rs_snapshot.benchmark_return_60d_pct,
                    rs_60d_pct=rs_snapshot.rs_60d_pct,
                    ...
                    rs_label=rs_snapshot.rs_label,
                    rs_experimental_20d_outperform_pass=bool(rs_snapshot.rs_20d_pct is not None and rs_snapshot.rs_20d_pct > 0),
                    rs_experimental_60d_outperform_pass=bool(rs_snapshot.rs_60d_pct is not None and rs_snapshot.rs_60d_pct > 0),
                    rs_experimental_percentile_pass=bool(rs_rank.get("rs_experimental_percentile_pass", False)),
    ...
    results.sort(
        key=lambda r: (
            ...
            -(r.rs_percentile_composite or -1.0),
