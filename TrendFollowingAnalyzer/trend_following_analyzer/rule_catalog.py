...
RULE_CATALOG = (
    ...
    RuleSpec("stock.prior_advance_window_proxy", EXPERIMENTAL, BACKTEST_ONLY, "Base 미탐지 시 최근 20거래일을 잠정 Base로 제외하고 이전 120거래일의 최대 시간순 저점→고점 상승을 측정"),
    ...
    RuleSpec("stock.price_data_discontinuity_check", EXPERIMENTAL, BACKTEST_ONLY, "수정주가/소스 불연속 가능성을 찾는 종가 일간 35% 초과 jump 품질 경고; 후보 필터가 아님"),
    RuleSpec("stock.base_structure", LECTURE_CORE, BACKTEST_ONLY, "강한 선행 상승 이후 베이스를 만들고 돌파를 기다린다는 강의 원칙"),
    RuleSpec("stock.base_window", EXPERIMENTAL, BACKTEST_ONLY, "Generic Base V1의 15~60거래일 탐색창"),
