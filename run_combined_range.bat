...
echo [0/8] Preparing shared point-in-time liquidity universe...
...
echo.
echo [1/8] Running KJB on shared universe union...
call "%ROOT%KJBChartAnalyzer\run_swing_range.bat" "%DATE_RANGE%" "%TOP_N%" "%SORT_BY%"
if errorlevel 1 goto RUN_FAILED
cd /d "%ROOT%"

echo.
echo [2/8] Applying KJB point-in-time membership...
...
echo.
echo [3/8] Running Swing on shared universe union...
call "%ROOT%SwingChartProbabilityAnalyzer\run_swing_range.bat" "%DATE_RANGE%" "%TOP_N%" "%SORT_BY%"
if errorlevel 1 goto RUN_FAILED
cd /d "%ROOT%"

echo.
echo [4/8] Applying Swing point-in-time membership...
"%FILTER_PYTHON%" %FILTER_PREFIX% "%ROOT%scripts\filter_liquidity_membership.py" ^
...
echo.
echo [5/8] Running MA V3 stateful scale-in backtest...
call "%ROOT%MAChartAnalyzer\run_ma_range.bat" "%DATE_RANGE%" "%TOP_N%" "%SORT_BY%"
if errorlevel 1 goto RUN_FAILED
cd /d "%ROOT%"

echo.
echo [6/8] Validating MA V3 outputs...
...
echo.
echo [7/8] Collecting independent confirmed signals...
