...
- Analyzer가 생성한 Agent용 압축 후보 데이터
- 후보 종목의 압축 기술지표
- 사용자가 명시적으로 제공한 차트
...
최소 Base/Bull/Bear 관점을 내부적으로 비교한다. 확률 숫자가 데이터로 뒷받침되지 않으면 억지로 정밀한 확률을 만들지 않는다.
...
Adapted for Korean equities from TraderMonty's `technical-analyst` methodology in `claude-trading-skills` (MIT License). See `THIRD_PARTY_NOTICES.md`.
