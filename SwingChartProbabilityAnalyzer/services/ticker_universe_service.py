...
class TickerUniverseService:
    ...
    def get_universe(self, top_n: int = 0, sort_by: str = 'market_cap', include_etf: bool = False) -> List[TickerInfo]:
        ...
        if not include_etf:
            ...
            mask = market.str.contains('ETF|ETN|ETP', regex=True, na=False) | name.str.contains('ETF|ETN|ETP', regex=True, na=False) | name.str.match(prefix, na=False) | ~ticker.str.fullmatch(r'\d{6}', na=False)
            df = df[~mask].copy()
