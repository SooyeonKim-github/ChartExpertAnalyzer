...
8. 강한 업종 안에서 실제 대장인지 Sector Context로 확인한다
9. 오늘만 강한 종목과 며칠째 시장 중심인 종목을 Leader Persistence로 구분한다
...
Sector Context와 Persistence는 기존 100점 Leader Score에 억지로 합산하지 않고 독립 Context로 유지합니다. 따라서 Range 백테스트에서 기존 점수와 신규 Context의 효과를 따로 검증할 수 있습니다.
...
기본 Leader/Timing/Chase 조건은 유지하면서 Context를 추가합니다.

- `STRONG_CONFIRMED`: 기존 Strong 조건 + Context가 있으면 `강한 업종의 상위 종목`, `PERSISTENT_LEADER`, `EMERGING_LEADER` 중 하나로 확인
- `CONFIRMED`: 기존 Confirmed 조건 충족
- `WATCH`: Leader 점수는 높지만 Timing 부족, Chase Risk 과다, 또는 약한 업종 + 낮은 Persistence 조합
- `REJECT`: 그 외

Context 데이터가 없는 경우에는 기존 판정 기준을 유지해 데이터 소스 장애 때문에 후보가 일괄 탈락하지 않도록 했습니다.
...
Range 결과에는 Sector/Persistence 컬럼과 함께 `D+1`, `D+5`, `D+20`, `D+60` 종가 기준 수익률이 저장됩니다.

## 설정

모든 주요 가중치와 임계값은 `config/default.yaml`에서 변경할 수 있습니다.

특히 다음 블록을 독립적으로 튜닝할 수 있습니다.
...
기존 Leader/Timing 테스트에 더해 Sector Context와 Persistence 합성 테스트를 포함합니다.
