...
def _base_row(
    *,
    ...
    return {
        ...
        "status": _text(status),
        "score": _num(score),
        "timing_score": _num(timing_score),
        "market": _text(market),
        "signal": _text(signal),
...
def _dedupe_within_analyzer(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        if old is None:
            ...
            continue
        new_rank = (
            -1e18 if pd.isna(row["score"]) else row["score"],
            -1e18 if pd.isna(row["timing_score"]) else row["timing_score"],
        )
        old_rank = (
            -1e18 if pd.isna(old["score"]) else old["score"],
            -1e18 if pd.isna(old["timing_score"]) else old["timing_score"],
        )
