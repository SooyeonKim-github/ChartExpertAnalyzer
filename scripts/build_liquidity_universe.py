...
def _trading_dates(stock, start: pd.Timestamp, end: pd.Timestamp, lookback: int) -> list[pd.Timestamp]:
    ...
    for multiplier in (1, 2, 4):
        ...
        try:
            raw = stock.get_market_ohlcv_by_date(fetch_start.strftime("%Y%m%d"), end.strftime("%Y%m%d"), "005930", adjusted=True)
        except Exception:
            raw = pd.DataFrame()
        if raw is None or raw.empty:
            continue
...
def _add_names(stock, daily: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in out[["ticker", "market"]].drop_duplicates().iterrows():
        ...
        try:
            name = stock.get_etf_ticker_name(ticker) if market == "ETF" else stock.get_market_ticker_name(ticker)
...
def build_liquidity_universe(start: pd.Timestamp, end: pd.Timestamp, top_n: int = 200, lookback: int = 20,
    ...
    for idx, date in enumerate(dates, start=1):
        day = pd.concat([_fetch_snapshot(stock, date, market, cache_dir) for market in markets], ignore_index=True)
        frames.append(day)
    ...
    raw["avg_trading_value_20d"] = raw.groupby("ticker", group_keys=False)["trading_value"].rolling(window=lookback, min_periods=lookback).mean().reset_index(level=0, drop=True)
    if start == end:
        ...
        target_dates = [pd.Timestamp(d).normalize() for d in sorted(raw.loc[(raw["date"] >= start) & (raw["date"] <= end), "date"].dropna().unique())]
    selected = []
    for date in target_dates:
        day = raw[raw["date"].eq(date)].copy()
        day = day[day["avg_trading_value_20d"].notna() & day["avg_trading_value_20d"].gt(0)]
        day = day.sort_values(["avg_trading_value_20d", "trading_value", "ticker"], ascending=[False, False, True]).head(top_n)
        if day.empty:
            continue
    ...
    daily = _add_names(stock, pd.concat(selected, ignore_index=True))
    return daily[["date", "source_rank", "ticker", "name", "market", "trading_value", "avg_trading_value_20d", "universe_cutoff_value", "lookback_days", "volume", "close"]].sort_values(["date", "source_rank"]).reset_index(drop=True)
...
def _build_union_excel(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame({"Ticker": union["ticker"], "Name": union["name"], "시장": union["market"], "시가총액": pd.NA, "거래대금": union["avg_trading_value_20d"], "거래량": union["volume"], "최근20일평균거래대금": union["avg_trading_value_20d"], "당일거래대금": union["trading_value"], "유니버스최고순위": union["universe_best_rank"], "유니버스포함일수": union["universe_membership_days"], "최근포함일": union["date"].dt.strftime("%Y-%m-%d")})
