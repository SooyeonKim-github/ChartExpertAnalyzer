...
def _market_state(row: pd.Series, daily_top_n: int, cfg: dict[str, float]) -> tuple[str, int, bool, str]:
    ...
    # 급락 후 반등은 기존 volatile 성과가 좋았던 구간을 무조건 제거하지 않도록 별도 취급한다.
    if np.isfinite(dd20) and dd20 <= cfg['shock_drawdown']:
        if np.isfinite(ret5) and ret5 >= cfg['shock_rebound_5d']:
            return 'SHOCK_REBOUND', min(3, daily_top_n), True, 'large drawdown + 5D rebound; BOTH only'
        if np.isfinite(ret5) and ret5 <= cfg['shock_down_5d']:
            return 'RISK_OFF', min(1, daily_top_n), True, 'large drawdown still falling; max 1 BOTH'

    # 지수 방향과 무관하게 시장 내부 breadth가 극단적으로 약하면 방어 모드.
    if (
        np.isfinite(b20) and np.isfinite(b60)
        and b20 < cfg['riskoff_ma20'] and b60 < cfg['riskoff_ma60']
    ):
        return 'RISK_OFF', min(1, daily_top_n), True, 'very weak breadth; max 1 BOTH'

    if regime == 'downtrend':
        return 'RISK_OFF', min(1, daily_top_n), True, 'downtrend; max 1 BOTH'

    # volatile을 일괄 차단하지 않는다. 반등 확인 전에는 BOTH 중심 선택 모드로 축소한다.
    if regime == 'volatile':
        if np.isfinite(dd20) and dd20 <= -0.05 and np.isfinite(ret5) and ret5 > 0:
            return 'SHOCK_REBOUND', min(3, daily_top_n), True, 'volatile drawdown + positive 5D rebound; BOTH only'
        return 'SELECTIVE', min(3, daily_top_n), True, 'volatile; BOTH only'

    if regime == 'uptrend':
        if (
            np.isfinite(b20) and np.isfinite(b60)
            and b20 >= cfg['healthy_ma20'] and b60 >= cfg['healthy_ma60']
        ):
            return 'HEALTHY', daily_top_n, False, 'uptrend + healthy breadth; original TOP N allowed'
        return 'SELECTIVE', min(3, daily_top_n), True, 'uptrend but breadth not healthy; BOTH only'

    if regime == 'range':
        return 'SELECTIVE', min(3, daily_top_n), True, 'range; BOTH only'

    return 'SELECTIVE', min(3, daily_top_n), True, 'unknown/other context; conservative BOTH only'


def _build_daily_context(context: pd.DataFrame, daily_top_n: int, cfg: dict[str, float]) -> pd.DataFrame:
    x = context.copy()
    x['signal_date'] = pd.to_datetime(x['date'], errors='coerce').dt.normalize()
    x['market'] = x['market'].map(_normalize_market)
    x['market_regime'] = x.get('market_regime', 'unknown').fillna('unknown').astype(str).str.lower()
    ...
    x['market_filter_both_only'] = [s[2] for s in states]
    x['market_filter_reason'] = [s[3] for s in states]
    return x.sort_values(['signal_date', 'market']).reset_index(drop=True)


def _apply_filter(top: pd.DataFrame, daily_context: pd.DataFrame, daily_top_n: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    x = top.copy()
    x['signal_date'] = pd.to_datetime(x['signal_date'], errors='coerce').dt.normalize()
    x['market'] = x['market'].map(_normalize_market)

    context_cols = [
        'signal_date', 'market', 'market_regime',
        'index_close', 'index_return_5d', 'index_return_20d', 'index_drawdown_20d',
        'index_ma20_gap', 'index_ma60_gap', 'index_volatility_20d', 'index_volatility_120d',
        ...
        'market_filter_state', 'market_filter_max_positions',
        'market_filter_both_only', 'market_filter_reason',
    ]
    context_cols = [c for c in context_cols if c in daily_context.columns]

    # 기존 combined 결과의 옛 context 컬럼과 중복되지 않도록 최신 market context로 교체한다.
    replace_cols = [c for c in context_cols if c not in {'signal_date', 'market'} and c in x.columns]
    if replace_cols:
        x = x.drop(columns=replace_cols)
    x = x.merge(daily_context[context_cols], on=['signal_date', 'market'], how='left')
    ...
    x['market_filter_both_only'] = x['market_filter_both_only'].fillna(True).astype(bool)
    x['market_filter_reason'] = x['market_filter_reason'].fillna('missing market context; conservative BOTH only')

    x['market_filter_eligible'] = (~x['market_filter_both_only']) | x['source_type'].eq('BOTH')
    x['market_filter_rank'] = np.nan

    eligible_idx: list[int] = []
    for (_, _), grp in x[x['market_filter_eligible']].groupby(['signal_date', 'market'], sort=False):
        ranked = grp.sort_values(['combined_score', 'ticker'], ascending=[False, True])
    ...
    x['market_filter_selected'] = x.index.isin(eligible_idx)

    # 입력 자체가 기존 일별 TOP N이므로 보통 이 cap에 걸리지 않지만, KOSPI/KOSDAQ 혼합 시에도
    # 최종 일별 종목 수가 원래 TOP N을 넘지 않도록 한 번 더 제한한다.
    selected = x[x['market_filter_selected']].copy()
    if not selected.empty:
        selected = selected.sort_values(
            ['signal_date', 'combined_score', 'ticker'], ascending=[True, False, True]
        )
        selected['market_filter_global_rank'] = selected.groupby('signal_date').cumcount() + 1
        keep = selected[selected['market_filter_global_rank'] <= daily_top_n].index
    ...
    decisions = x.sort_values(['signal_date', 'combined_score'], ascending=[True, False]).reset_index(drop=True)
    selected = selected.reset_index(drop=True) if not selected.empty else selected
...
def _event_summary(cohorts: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for cohort, frame in cohorts.items():
        for h in MILESTONES:
            col = f'ret_D{h}'
            v = pd.to_numeric(frame[col], errors='coerce').dropna() if col in frame.columns else pd.Series(dtype=float)
            rows.append({
                'cohort': cohort,
                'horizon': h,
...
def _date_summary(cohorts: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for cohort, frame in cohorts.items():
        for h in MILESTONES:
            col = f'ret_D{h}'
            basket = frame.groupby('signal_date')[col].mean().dropna() if (not frame.empty and col in frame.columns) else pd.Series(dtype=float)
            rows.append({
                'cohort': cohort,
                'horizon': h,
...
def _write_summary(path: Path, event: pd.DataFrame, date: pd.DataFrame, daily: pd.DataFrame, daily_top_n: int, cfg: dict[str, float]) -> None:
    lines = [
        '# Market Filter Backtest',
        '',
        '기존 Analyzer/Combined Score/COMBINED_TOP 후보는 변경하지 않고 마지막 단계에서 시장 상태로 신규진입 수만 제한한다.',
        '',
        '## Filter Rules',
        '',
        f"- HEALTHY: uptrend + breadth(MA20>={cfg['healthy_ma20']:.0%}, MA60>={cfg['healthy_ma60']:.0%}) -> 기존 TOP{daily_top_n} 허용",
        '- SELECTIVE: range/volatile/약한 breadth -> 기존 TOP 후보 중 BOTH만 최대 3개',
        f"- SHOCK_REBOUND: 20D drawdown<={cfg['shock_drawdown']:.0%} + 5D rebound>={cfg['shock_rebound_5d']:.0%} -> BOTH 최대 3개",
        '- RISK_OFF: downtrend/급락 지속/극단적 weak breadth -> BOTH 최대 1개',
    ...
    for cohort in [f'COMBINED_TOP{daily_top_n}', 'MARKET_FILTERED']:
        vals = []
        for h in (5, 10, 20):
            row = event[(event['cohort'] == cohort) & (event['horizon'] == h)]
    ...
    if not daily.empty:
        counts = daily.groupby('market_filter_state')['signal_date'].nunique().sort_values(ascending=False)
...
def main() -> int:
    p = argparse.ArgumentParser(description='Apply market exposure filter to existing Combined TOP N results')
    ...
    decisions.to_csv(combined_dir / 'market_filter_decisions.csv', index=False, encoding='utf-8-sig')
    selected.to_csv(combined_dir / 'market_filtered_candidates.csv', index=False, encoding='utf-8-sig')
    daily.to_csv(combined_dir / 'market_filter_daily.csv', index=False, encoding='utf-8-sig')
    event.to_csv(combined_dir / 'performance_market_filter_event.csv', index=False, encoding='utf-8-sig')
    date.to_csv(combined_dir / 'performance_market_filter_date_equal.csv', index=False, encoding='utf-8-sig')

    summary_path = combined_dir / 'market_filter_summary.md'
    _write_summary(summary_path, event, date, daily, args.daily_top_n, cfg)
