...
_INDEX_ALIASES={"^KS11":"KOSPI","KOSPI":"KOSPI","1001":"KOSPI","^KQ11":"KOSDAQ","KOSDAQ":"KOSDAQ","2001":"KOSDAQ"}
...
class PykrxDataProvider(DataProvider):
    ...
    def _cache_path(self,ticker,start_date,end_date,kind):
        safe=str(ticker).replace('^','').replace('/','_')
        digest=hashlib.md5(f'{kind}_{ticker}_{start_date}_{end_date}'.encode()).hexdigest()[:10]
    ...
    def get_ohlcv_by_date(self,ticker,start_date,end_date):
        ...
        kind='naver_index' if alias in _INDEX_ALIASES else 'stock_or_etf'
        ...
        if alias in _INDEX_ALIASES:
            ...
            stock=self._load_pykrx(); start=pd.Timestamp(start_date).strftime('%Y%m%d'); end=pd.Timestamp(end_date).strftime('%Y%m%d'); code=self.normalize_ticker(alias)
