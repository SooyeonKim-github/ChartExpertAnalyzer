...
) else if exist "%LIQ_ROOT%MAChartAnalyzer\.venv\Scripts\python.exe" (
    set "LIQ_PYTHON_EXE=%LIQ_ROOT%MAChartAnalyzer\.venv\Scripts\python.exe"
) else (
    where py >nul 2>nul
    if not errorlevel 1 (
...
if "%LIQ_TARGET%"=="" (
    "%LIQ_PYTHON_EXE%" %LIQ_PYTHON_PREFIX% "%LIQ_ROOT%scripts\build_liquidity_universe.py" ^
        --top-n %LIQ_TOP_N% --lookback %LIQ_LOOKBACK% --output-dir "%LIQ_OUT_DIR%"
) else (
    "%LIQ_PYTHON_EXE%" %LIQ_PYTHON_PREFIX% "%LIQ_ROOT%scripts\build_liquidity_universe.py" ^
        --as-of "%LIQ_TARGET%" --top-n %LIQ_TOP_N% --lookback %LIQ_LOOKBACK% --output-dir "%LIQ_OUT_DIR%"
)
if errorlevel 1 (
...
"%LIQ_PYTHON_EXE%" %LIQ_PYTHON_PREFIX% "%LIQ_ROOT%scripts\build_liquidity_universe.py" ^
    --date-range "%LIQ_TARGET%" --top-n %LIQ_TOP_N% --lookback %LIQ_LOOKBACK% --output-dir "%LIQ_OUT_DIR%"
if errorlevel 1 (
