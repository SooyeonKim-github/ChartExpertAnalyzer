...
class ArticleRepository:
    COLUMNS = [
        ...
        "url_hash","title_hash","content_hash","duplicate_of","body_length","has_full_body",
        "fetch_status","http_status","parse_status","error_code","material_candidate",
        "analysis_status","processed_at","source_metadata_json"
    ]
    ...
    def get_by_article_id(self, article_id: str):
        with self.database.connect() as conn:
            return conn.execute("SELECT * FROM articles WHERE article_id = ? LIMIT 1", (article_id,)).fetchone()
    ...
    def upsert(self, article: RawArticle) -> str:
        ...
        assignments.extend([
            ...
            "WHEN excluded.first_seen_at < articles.first_seen_at THEN excluded.first_seen_at "
            "ELSE articles.first_seen_at END",
            "last_seen_at=CASE "
            ...
            "WHEN excluded.last_seen_at > articles.last_seen_at THEN excluded.last_seen_at "
            "ELSE articles.last_seen_at END",
