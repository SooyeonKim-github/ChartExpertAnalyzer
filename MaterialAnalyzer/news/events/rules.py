...
EVENT_RULES = [
    ...
    ("LITIGATION", ("소송등의판결", "소송 등의 판결", "소송등의제기", "경영권분쟁소송", "가처분", "소송 제기", "소송제기")),
    ("MARKET_QUERY", ("조회공시요구", "풍문또는보도", "풍문 또는 보도")),
    ("DEBT_GUARANTEE", ("타인에대한채무보증결정", "채무보증결정", "채무 보증", "타인에대한담보제공결정", "담보제공결정")),
    ("IR_EVENT", ("기업설명회", "ir개최", "ir 개최")),
    ("OWNERSHIP_CHANGE", ("최대주주변경", "최대주주 변경", "경영권 변경")),
    ("INSIDER_OWNERSHIP", ("임원 주요주주특정증권등소유상황보고서", "주식등의대량보유상황보고서")),
    ("CORPORATE_GOVERNANCE", ("주주총회소집결의", "임시주주총회결과", "정기주주총회결과", "주주명부폐쇄기간", "기준일설정", "대표이사변경", "대표이사(대표집행임원)변경", "임원선임", "감사선임")),

    # DART high-value corporate actions that were previously left as UNKNOWN.
    ("VALUE_UP", ("기업가치제고계획", "기업가치 제고계획", "밸류업 계획", "밸류업계획")),
    ("TREASURY_STOCK_DISPOSAL", ("자기주식처분결정", "자기주식 처분결정", "자기주식처분결과", "자사주 처분")),
    ("CONVERTIBLE_EXERCISE", ("전환청구권행사", "신주인수권행사", "교환청구권행사")),
    ("CONVERTIBLE_ADJUSTMENT", ("전환가액의조정", "전환가액 조정", "행사가액의조정", "행사가액 조정", "교환가액의조정", "교환가액 조정")),
    ("CAPITAL_REDUCTION", ("감자결정", "자본감소", "자본 감소")),
    ("SHARE_CONSOLIDATION", ("주식병합결정", "주식병합", "주식분할결정", "주식분할")),
    ("ASSET_TRANSACTION", ("유형자산양수결정", "유형자산양도결정", "유형자산 양수", "유형자산 양도", "자산양수", "자산양도")),
    ("INVESTMENT_DISPOSAL", ("타법인주식및출자증권처분결정", "타법인주식및출자증권양도결정", "타법인 주식 처분", "출자증권 처분")),
    ("FINANCING", ("금전대여결정", "금전 대여", "단기차입금증가결정", "단기차입", "자금차입", "자금 차입")),
    ("DERIVATIVE_LOSS", ("파생상품거래손실발생", "파생상품 거래손실", "파생상품손실")),
    ("RESTRUCTURING", ("회생절차개시", "회생절차", "파산신청", "파산 선고", "파산선고")),
    ("MATERIAL_MANAGEMENT", ("투자판단관련주요경영사항", "투자판단 관련 주요경영사항")),
    ("PUBLIC_OFFER", ("공개매수신고서", "공개매수결과보고서", "공개매수")),

    ("ORDER_CONTRACT", ("단일판매", "공급계약", "수주", "계약체결", "납품계약", "도급계약")),
    ("CAPITAL_RAISE", ("유상증자", "무상증자", "전환사채", "신주인수권", "교환사채", "제3자배정", "소액공모공시서류", "증권발행결과")),
    ("MNA", ("합병", "분할", "인수", "영업양수", "영업양도", "m&a", "타법인주식및출자증권취득결정", "타법인주식및출자증권양수결정")),
    ("CAPEX", ("신규시설투자", "시설투자", "증설", "공장건설", "설비투자", "capex")),
    ("INVESTMENT", ("투자유치", "투자 유치", "투자계획", "투자 계획", "투자 결정", "투자협약", "출자결정", "지분투자", "특수관계인에대한출자")),
    ("EARNINGS", ("잠정실적", "영업(잠정)실적", "영업 실적", "영업실적", "매출액", "손익구조", "실적발표", "실적 발표")),
    ("GUIDANCE", ("실적전망", "가이던스", "매출전망", "영업이익전망")),
    ("DIVIDEND", ("현금배당", "현물배당", "배당결정", "배당 결정")),
    ("BUYBACK", ("자기주식취득", "자사주취득", "자기주식소각", "주식소각", "자사주 소각", "신탁계약해지결과보고서", "신탁계약에의한취득상황보고서")),
...
STAGE_RULES = [
    ...
    ("COMPLETED", ("완료", "준공", "종료", "최종 완료", "결과보고")),
    ("STARTED", ("가동", "착공", "개시", "양산 시작", "판매 시작", "출시", "행사")),
    ("PLANNED", ("계획", "예정", "목표", "추진", "검토", "로드맵")),
    ("APPROVED", ("승인", "허가", "인가")),
    ("CONFIRMED", ("체결", "수주", "결정", "확정", "선정", "지정", "취득", "계약", "조정")),
]
...
POSITIVE_KEYWORDS = (
    ...
    "증설", "주식소각", "자사주 소각", "특허취득", "실적개선", "흑자전환", "기업가치제고",
)
...
def infer_polarity(text: str, event_type: str, event_stage: str = "UNKNOWN") -> str:
    ...
    if any(normalize_rule_text(keyword) in compact for keyword in NEGATIVE_KEYWORDS):
        return "NEGATIVE"
    if event_type in {"TRADING_HALT", "SANCTION", "DERIVATIVE_LOSS", "RESTRUCTURING"}:
        return "NEGATIVE"
