...
class HttpClient:
    ...
    def get(self, url: str, *, params: Optional[Dict[str, Any]] = None, headers: Optional[Dict[str, str]] = None) -> FetchedContent:
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                response = self.session.get(url, params=params, headers=headers, timeout=self.timeout_sec)
                ...
                return FetchedContent(url=response.url, status_code=response.status_code, content_type=response.headers.get("Content-Type", ""), text=response.text, raw_bytes=response.content, fetched_at=datetime.now(timezone.utc), encoding=response.encoding)
        ...
        raise FetchError(f"request failed url={url}: {last_error}")
